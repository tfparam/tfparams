...
|------|---------|-------------|
| `--plan-json` | stdin | Plan JSON file (`terraform show -json <planfile>`). `--state-json` is a deprecated alias |
...
| `--output-mode` | `standalone` | `standalone` / `inject` / `replace` |
...
| `--sort-by` | `name` | `name` / `required` / `type` |
